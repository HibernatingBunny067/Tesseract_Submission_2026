...
def clinical_interpreter_node(state: DesignState) -> dict:
    ...
    try:
        ...
        print(f"[ClinicalInterpreter] LLM failed ({e}), falling back to local NLP parser")
        try:
            ...
            print(f"[ClinicalInterpreter] Fallback also failed: {fallback_err}")
            traceback.print_exc()
...
def materials_advisor_node(state: DesignState) -> dict:
    ...
    try:
        ...
        print(f"[MaterialsAdvisor] LLM failed ({e}), using heuristic fallback")
...
def optimization_controller_node(state: DesignState) -> dict:
    ...
    if corrections and attempt > 1:
        ...
        opt_kwargs["max_steps"] = corrections.get("adjusted_max_steps", 3)
    ...
    try:
        ...
        print(f"[OptimizationController] Error during optimization: {e}")
        traceback.print_exc()
        messages.append(create_message(
            "optimization_controller",
...
def _generate_corrections(
    ...
    try:
        ...
        print(f"[ValidationAuditor] LLM correction failed ({e}), using heuristic")
